...
router.post('/send', authorizeKey, verifyUser, async (request, response, next) => {
  try {
    ...
    if (notificationType === 'book_enquiry_notification') {
      ...
      if (senderUserName === undefined || senderUserName === null || senderUserName === '') {
        response.status(403).json({
          result: {
      ...
      await NotificationsCollection.insertOne(notificationData, async (error, result) => {
        if (error) {
          response.status(500).json({
            result: {
